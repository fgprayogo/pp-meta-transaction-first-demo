...
const user2WalletAddr = process.env.REACT_APP_USER2_ADDRESS
...
function User2() {
    ...
    const loadPidrBalance = async () => {
        const _pidrBalance = await pidrContract.connect(deployerAcc).balanceOf(user2WalletAddr);
    ...
    const loadNFTCollections = async () => {
        const _nftBalance = await planetPersibNFTContract.connect(deployerAcc).balanceOf(user2WalletAddr)
        let _tokenId = []
        for (let i = 0; i < _nftBalance; i++) {
            const _nftBalanceId = await planetPersibNFTContract.connect(deployerAcc).tokenOfOwnerByIndex(user2WalletAddr, i)
    ...
    return (
        <Box display="flex"
            ...
            <>
                ...
                <h1>USER 2 PAGE</h1>
                <h3>Wallet Address : {user2WalletAddr}</h3>
